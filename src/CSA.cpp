                    ...
                    for (auto ap : aps)
                    {
                        ...
                        if (ap.beacon)
                        {
                            ...
                            for (int i = 0; i < 4; i++)
                            {
                                ...
                                beacon.addr1(config.target_mac);
                                beacon.addr2(addr);
                                beacon.addr3(beacon.addr2());

                                // for (auto option : ap.beacon->options())
                                // {
                                //     beacon.add_option(option);
                                // }

                                uint8_t new_channel = 1;

                                uint8_t now_channel = ap.beacon->ds_parameter_set();
